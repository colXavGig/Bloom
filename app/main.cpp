...
int main(int argc, char** argv){
    ...
    cout<<"succes";
        ...
        index.createNewBranch(argv[2],index.getSignature());
    }
    else if (cmd == "check"){
        index.setCurrentBranch(argv[2]);

        FileBuilder *fb = new FileBuilder(&paths);
     
        try{
            fs::path p = paths.getGardenTagPath(index.getSignature());
            cout << p<<"\n";
...
void commit(Index& index, GardenPath *paths, char** arr, int size) {
    ...
    index.commit(fw.writeToFile(&HTree, tag_msg,index.getSignature()));
